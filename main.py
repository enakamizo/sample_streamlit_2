...
st.title('Streamlit_eikoの部屋') # タイトル
...
st.dataframe(df.style.highlight_max(axis=0), width=200, height=200) # データフレームの表示（スタイル付き）
...
st.title('Streamlit 画像') # タイトル

st.write('kokoro on the beach') #ウィジェットの表示

img = Image.open('kokoro on the beach.jpg') # 画像の読み込み
st.image(img, caption='初めて海を見たよ', use_column_width=True) # 画像の表示

#----------------------------------------------------#
st.title('Streamlit インタラクティブ') # タイトル

st.write('Interactive Widgets') #画像の表示

text = st.sidebar.text_input("あなたの趣味を教えてください。") # テキスト入力
"あなたの趣味は", text,"です。"

condition = st.sidebar.slider("あなたの今の調子は？", 0,100,80) # スライダー
"コンディション：", condition
...
slider_text = st.slider("スライダーで数字を決定できます。",0,100,50) # (最小、最大値、デフォルト値)の順で設定されます。
...
st.download_button(label='ダウンロードはこちら', 
                   data=output_text, 
